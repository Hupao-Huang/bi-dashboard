...
## v1.77.8 (2026-06-26) — 合思审批助手再升级（付款截图核对 / 境外免票 / 部门口径修正）+ 凭证多账簿 + 业务报表网页上传

本版收三摊已上线功能：合思日常报销审批助手新增两条规则 + 部门口径修正、财务凭证查询支持多账簿合并、财务业务报表支持网页上传。均不动看板既有数据口径。

**合思日常报销 AI 审批建议（财务同事可见）**
- 新增「付款截图 vs 发票金额」核对：自动识别报销单付款截图的实付金额，与发票价税合计比对，实付明显超过发票的转人工复核（含手续费/凑整容差，差旅有票无截图不冤枉）。
- 新增「境外报销免发票」豁免：识别到外币/汇率的付款截图、又无中国发票时，不再误判「缺发票」自动放行（关键词已收紧防误判）。
- 部门口径修正：审批建议改读单据上实际填写的提交人/费用承担部门（不再读员工默认部门）；规则 1（发起人部门）取消、规则 2 改为「费用承担部门不能选公司（顶层法人实体）」、规则 10/13（研发/品牌）统一同口径。
- 修复：单据在合思删除的费用明细行，同步后不再残留显示（对称的明细级清理）。

**财务「凭证查询」：账簿支持多选合并**
- 账簿从单选改多选，多个账簿的凭证合并成一张表展示（带账簿列）。默认每本先取一部分（快），需要全部时点「拉全」。
- 账簿选择器换成仿用友弹窗：左侧全账簿勾选、右侧已选面板可单删、按编码/名称搜索、全选/清空、确定才生效。

**财务「业务报表」：支持网页上传 Excel**
- 业务预决算报表新增网页上传，不必再走后台 CLI。支持全量/增量两种模式，上传前逐格 diff 预览变更（按渠道分组、被删渠道可见），独立导入权限。

底层付款截图核对、外币豁免、部门口径修正均经 /code-review 二审 + 真实单据 dry-run 回测；业务报表上传经逐任务 TDD + 二审（修一个 diff 误标删行的财务红线 bug）。

---

## v1.77.7 (2026-06-24) — 小红书看板升级（明细全量 + 排序翻页 + 自定义列 + 导出 Excel + 乘风看板）

补记：本版当时只发了站内公告，未写 CHANGELOG 段落，现补齐对齐。

- 小红书千帆/乘风明细表去掉「只显示前 50 条」限制，当前筛选下全量笔记/商品分页查看。
- 数值列点表头升/降序排序，笔记支持按发布时间排。
- 「自定义指标」可勾选/拖拽列、存常用方案一键调出。
- 新增「下载」按钮，把当前筛选 + 已选列全量导出 Excel（表头加粗、数值列导真数字）。
- 新增「乘风」菜单（信息流投流数据），与千帆并列，口径/样式/自定义指标一致。

---

## v1.77.6 (2026-06-18) — 张俊对外付款单 AI 审批建议上线 + 合思审批助手一批迭代

本版收两摊：补记此前漏入 CHANGELOG 的「张俊对外付款单 AI 审批」大功能 + v1.77.5 之后的合思审批迭代。均为合思审批助手内部能力，不动看板数据与其他功能。

**张俊对外付款单 AI 审批建议（全新，dry-run 只给建议不真审批）**
- 触发：当前审批人为张俊的对外付款单（B 开头）/预付款单（J 开头，先款后票，走规则子集），独立审批引擎，与樊雪娇日常报销互斥。
- 已上线 14 条规则：所属公司 / 部门末级 / 客户多选 / 付款事由 / 收款合规 / 防自付 / 明细 / 阳光天际→悦伍 / 大额合同提醒 / 金额上限 / 收款方=开票方 / 购买方=所属公司 / 发票张数对账 / 防重复报销。
- 张俊真实付款单回测健康（通过/人工/驳回三档分布合理、无误报）；查看张俊待审时显示只读规则卡（仿樊雪娇）。
- 剩 8 条规则（品牌/研发费用类型清单、检测费、装修费、无票识别、核销结构）等业务口径确认后再补，故本版按 PATCH 收。

**AI 审批建议扩到更多财务审批人**
- 日常报销建议触发审批人扩到 金海侠 / 周翻翻 / 张勇（+樊雪娇）；付款单扩到 苏安妮（+张俊）。规则引擎不变，仅放开触发审批人。三人已注册 BI + 绑合思工号，登录即见建议。

**樊雪娇日常报销 2 条新规则**
- 付款截图认单据附件（不止明细自带）；定额发票金额读不出转人工（不再误判）。

**合思单据数据新鲜度（改了/删了的单据不再显示旧内容）**
- 单据驳回改动后发票残留致误报 → 同步时按整对清理残留发票；单据删除/撤回后合思接口不再返回、永久冻结「待审」→ 据审批状态标记退场（重新提交自愈）。配 15 分钟高频同步缩短「改后还显旧」窗口。实测当场退 34 张幽灵单，set-diff 验证无真僵尸。

**规则 7-1 交通差旅口径调整 + 几个修复**
- 规则 7-1：整体豁免关联出差申请单仅集团生效；集团下私车公用/过路费也须挂出差申请单且不超额度。
- 预付款单「预付事由」读 title 不读 description，修 A5 误报「事由为空」+ A13 漏判。
- 张俊付款单待审列表发票查询 3 次合并为 1 次（性能）。
- 修 AntD 固定列横滚 hover 时操作列「透叠」压住 AI 建议列；规则展示卡按规则号升序、两张表加规则号列。

底层均经 /code-review 二审 + dry-run 回测探针（改前存 baseline → 改后逐单 diff=0）验证。

---

## v1.77.5 (2026-06-17) — 过路费普票通行日期补全（接 v1.77.4 条3）

过路费报销发现第二种发票格式，本次补全（跑哥 2026-06-17）：

- **普通电子发票通行日期在「货物明细行」**：v1.77.4 只解析了全电票的「备注」；普票（ELECTRONIC_PAPER_FEE 等）备注为空，通行日期在发票货物明细行 `E_system_发票明细_通行日期起/止`（毫秒戳）。sync-hesi 主体同步后，对「待审单 + 过路费明细 + `toll_pass_start` 仍空」的发票额外调 detailBatch 取明细行日期回补；幂等只补空值，不覆盖全电备注已解析的结果。
- **两种票互补、无法统一**：探针实测确认全电票明细行无通行日期字段（走备注）、普票备注为空（走明细行），两条路并存。
- /code-review 二审：回补阶段重取 token，防 sync 跑久末尾 token 过期致普票静默漏补；`asMs` 兼容数字/字符串/json.Number。
- 实测：待审普票 41 张通行日期 NULL → 全部补真实日期（对比基线，命中 41/41）；只换 `sync-hesi.exe`，bi-server 未动。

只影响合思审批助手过路费补贴判定，不动看板数据与其他功能。

---

## v1.77.4 (2026-06-17) — 合思审批助手规则批次（差旅补贴口径 + 发票时效 + 黄涛特批）

合思日常报销单 AI 审批建议本次一批规则改动（樊雪娇口径，跑哥 2026-06-17）：

- **私车公用消费日按行车记录时间**：不再用明细自填日期，改用行车记录的实际出行日期（条2）。
- **过路费消费日按发票通行日期**：从发票备注「通行日期起/止」解析；发票表加 `toll_pass_start/end` 两列，sync-hesi 解析（CST 固定时区），回补只针对待审单收敛（条3）。
- **补贴按职级扣交通补**：私车/过路消费日落在出差申请单期间内的，集团经理及以上扣 70/天、其他扣 50/天（条4）。
- **发票时效用首次提交时间**：规则 8-4/14 改用表 `submit_date`（退回重提不刷新），避免退回重提把合规发票误判超期。
- **集团黄涛特批豁免**：收款户名「黄涛」+ 实际报销额 ≤ 该单发票合计 → 跳过全部规则直接通过（条5，跑哥认可户名同名风险）。
- /code-review 二审修复：COALESCE 防覆盖已解析通行日期、`fee_type_id` 加索引、`expenseMoney>0` 守卫、无限重拉收敛。
- 口径偏严取舍：没关联出差申请单时私车/过路日仍扣（防漏）；行车记录缓存冷时兜底明细日期。

只影响合思审批助手 AI 建议，不动看板数据与其他功能。commit `9df8e3e`。

---

## v1.77.3 (2026-06-16) — 工具箱「新增采购订单」+ 财务「凭证查询」+ 数据关联三维图 + 运维任务名中文化

**工具箱新增「采购订单」(YS 工具)**: 导入采购申请 Excel 模板 → 自动把组织/供应商/物料名称翻译成用友编码、按货品档案税率算好价税、带上计划到货日期 → 预览核对(逐行红绿标注、可点某张订单只看它的明细) → 一键在用友批量建采购订单(开立态)。全程实时对接用友、防重复提交。**税率以用友货品档案为准**, 模板填的不一致会橙色提醒(如矿盐模板填 9% 但货品档案是 13%, 自动按 13% 算)。配套供应商字典每日 05:00 自动同步(BI-SyncYSVendors)。底层经 /code-review 二审修复(价税精度/严格判建单成功/防呆/抽共享包)

**财务新增「凭证查询」(仅超管)**: 按账簿、账期实时查询用友会计凭证, 列表看凭证头(字号/类型/摘要/借贷合计/制单人/状态), 点开看分录明细(科目/辅助核算/借贷金额)。复用合思那套 QueryVoucherList, 14 个法人主体账簿可选

**数据关联三维图 (仅超管)**: 新增可旋转的 3D 数据架构全景图(react-force-graph-3d), 看"数据从哪来(吉客云/用友/合思/钉钉/RPA) → 汇成哪些数据 → 喂给哪些看板"

**运维监控任务名中文化**: 定时任务列表补齐 8 个任务的中文名, 不再显示 schtasks 英文原名

---

## v1.77.2 (2026-06-15) — 客服服务分/评论实时同步 + 综合看板环比同比 + 审批列表打开更顺

**客服服务分 / 评论数据 实时同步 (新增 2 个 RPA 回调接口)**: RPA 抓完数据写完文件后立即回调触发导入(`/api/webhook/sync-service-score` + `/api/webhook/sync-comment`, 同步执行秒级返回, 复用 webhook 密钥 + 各自独立并发锁, 无参数全量扫目录幂等), 修"文件下午才落地(14:53-18:44)、当天 13:00 兜底导入赶不上、要等晚上 19:30 定时任务才入库"的延迟。原定时任务保留当兜底, 双保险。各加 3 边界单测(403/405/409), 端到端实测通过(服务分 1536 条 / 评论 168 文件 3994 条)

**综合看板 KPI 卡新增环比/同比**: 销售额等卡片加上环比上期、同比去年, 三卡布局不变

**店铺数据概览 总销售额拆分**: 总销售额拆成 销售额 / 调拨额 两部分展示

**合思待审批列表打开更顺**: 私车公用行车记录查询改非阻塞 + 启动预热, 修复冷缓存下打开审批列表偶发卡死(datalink 接口冷拉现拉阻塞同款病根绝)

**SKU 动销率财务看板 (超管)**: 新增 SKU 动销率分析页, 当前仅超级管理员可见, 暂不开放财务角色

**内部清理 (用户无感知)**: 全站文字色统一到设计令牌、清除一批死代码(12 前端僵尸页 + 36 不可达 Go 函数/类型 + 42 死测试 + 3 孤儿组件 + 卸未用依赖)、侧边栏底部用户卡去重复分割线

---

## v1.77.1 (2026-06-12) — 全仓代码体检修复: AI 审批建议更稳 + 待审批列表提速 + 一批数据完整性加固

**背景**: 全仓 Go 代码 max 档审查(9 视角侦查+逐条独立核验, 报告 CODE_REVIEW_20260612.md), 15 条发现分三批全部闭环, 每批经 /code-review 二审。

**合思 AI 审批建议更稳 (4 个判定 bug)**:
- 规则7-3: 住宿明细缺起始日期时间夜数会算成 2 万+天导致超标住宿永不触发, 加守卫(与 extractFeeDates 同口径: start>1e12+区间封顶370天), 区间异常转人工
- 规则12-1: 私车公用金额 0/解析失败不再静默通过(0≤补助恒成立), 转人工核金额
- 规则8/10/15/16: 发票数据读取失败/不完整时不再拿残缺数据下结论(会漏驳也会误驳), 整体降级转人工; 规则10 费用类型缺失转人工不再误驳
- 共 7 个回归单测锁行为

**待审批列表提速 ~10 倍**: 部门末级/研发/品牌中心判定从每单最多 22 条递归 SQL 改进程级缓存(511 行部门表 5min TTL 一次加载), 一刷列表从 ~2200 条查询(卡 2-3 秒)降到最多 1 条; 语义零变化有对照测试锁定

**AI 智能助手加权限闸**: 新权限点"AI 智能助手"(后端 4 接口+前端浮窗), 原来任何登录用户可 API 直调拿全公司数据口径; 默认仅超管, 其他角色在角色管理勾选开通

**数据完整性加固 (rows.Err 家族 8 处)**: 评论列表/下拉/导出(导出在写响应前拦截, 防中途断连导出"看起来完整实缺一半"的 xlsx) + 合思附件统计/行程订单 + 销售单 schema 同步(残缺清单本轮跳过); 另: 吉客云翻页游标缺失守卫(防原地重拉)、缓存包装层补 Unwrap/Flush(生成计划卡死同款病第二例根绝)

**特殊渠道清单收敛**: 京东/猫超/朴朴/小象/叮咚 渠道口径(店名/shopID/仓号/部门/价格门控)从散落 6 处硬编码收敛成 internal/specialchannel 注册表单一来源 + 黄金测试锁值; 未来加新渠道(如七鲜)从改 6-9 个文件变成注册表加一行

**导入数字解析统一**: 5 个平台导入工具各自实现的数字清洗(¥/￥/元/%/千分位)统一到公共解析, 消除"同一格式 A 平台解析正常 B 平台静默按 0"的跨平台口径漂移; 严格超集, 原结果不变

**审批参数配置化**: 住宿费标准矩阵(职级×城市)/出差补贴标准搬进数据库配置表, 财务调整口径改一条记录 5 分钟生效, 不再需要改代码编译部署; 配置缺档/JSON 异常/表故障三级回落代码默认值

**运维**: "运营数据导入"13:00 定时任务连挂 3 天根治 — bat 脚本 LF 换行+中文注释让 cmd 解析错位秒崩(exit 255), 全仓 24 个 bat 规范化 CRLF+纯ASCII + .gitattributes 防复发 (数据未受损, RPA 正常路径每天 04:18 已导入, 13:00 仅兜底)

---

## v1.77.0 (2026-06-12) — 合思单据详情大升级(审批流/行车记录) + AI 审批规则一批新口径 + 服务分管理增强

**合思 · 单据详情「审批流」页签**: 详情弹窗新增审批流时间线 — 每个节点显示审批人/审批意见/耗时, 当前停在哪个节点一目了然; 跳过的节点正确翻译显示, 评论里@人的 ID 占位符反查成真实姓名, 与左侧标签重复的"同意"意见行去重不再刷屏。

**合思 · 私车公用看得见行车记录**: 报销明细里直接展示钉钉行车记录 — 出发地→目的地/里程/每公里补助标准/系统算出的应补金额, 跟报销金额当场对账。(坑: 行车记录接口默认只回未使用记录, 已改为拉全量)

**合思 · 详情排版重构**: 基本信息整页重排, 关联申请单从字段表里拆出来做成独立卡片区; 行程消费信息从纯文字流改成结构化列表, 并按特征自动推断订单类型(火车/飞机/酒店/用车), 不再笼统显示"订单"。主体校验补分公司豁免 — 分公司合同用主公司主体不再误报。

**合思机器人 · AI 审批规则一批新口径 (樊雪娇 6/11-6/12 确认)**:
- **规则12-1 私车公用自动对账**: 报销金额 ≤ 行车记录算出的补助 → 自动通过, 超了建议驳回, 拉不到记录才转人工 — 满屏"[需人工核]自驾"成为历史 (dry-run 300 单零误报); 顺手把 15-4 油费判定从按明细改成整单聚合(财务"总额"口径), 误驳大降
- **规则15 线下(世创/世用)专属规则集**: 非专票要付款截图/金额与发票一分不差/线下补贴 120÷100 折算/私车公用油费需≥报销额; 经二审修 3 处(私车日与补贴期间求交集/字典查不到留痕/日期区间封顶)
- **规则16 企业支付行程防重复报销**: 出差申请单下企业支付的车票, 同车次+同乘车人又出现在报销发票 → 票面价一致建议驳回, 不一致转人工; 排除退票/退订, 蓝军审查加固
- **规则17 补贴日期校验**: 补贴日期跟申请单起止比对 — 单日严判、区间有交集即过, 防按月报销被误杀 (dry-run 误报 12→2)
- **规则18 广告费发票校验**: 发票项目名称须含"广告/推广"字样, 现场调合思拉发票货物行比对 (dry-run 抓到摆摊桌/石膏制品/印刷品等真问题票)
- **付款截图豁免**: 交通及差旅费类型不再要求付款截图; 火车票判定文案重写更易读
- 规则判定卡只在查看樊雪娇待审批时展示; 自己审自己的单不再发钉钉通知; 未绑定合思工号的同事"看自己"加真名兜底

**客服 · 服务分管理增强**: 每格分数加环比涨跌箭头, 日期组间加粗分隔线, 每天的变化一眼可辨; 支持导出 Excel 和修改分数(独立对照表, RPA 每日重导不冲掉人工修改, 恢复原始仅管理员)。

**数据修复**: 合思附件批量参数 200 超接口上限导致附件漏同步三周, 改回 100 并补齐 (教训: 调大 API 批量参数必须实查响应体确认生效)。

---

## v1.76.7 (2026-06-11) — 客服服务分管理上线 + 合思机器人审批体验大改

**客服·服务分管理 (新页面)**: 各平台店铺服务分每日跟踪 — 平台 Tab + 店铺×日期透视表(日期隔条纹、服务分列突出、低于目标标红) + 达标统计 + 店铺 30 天走势弹窗。三项分数按平台显示真实含义(京东自营=响应时间/应答率/满意度, 拼多多=发货/物流/商品/服务/基础分), 数据每天自动同步。

**合思机器人 · 审批更准**: 修复"点了审批单子还在列表"的三个根因 — 支付环节的单不再混进待审批、审批后等合思真正办完再更新状态、驳回此前从未真正生效的问题已根治(参数修正)。驳回支持选择「驳回节点」和「重审路径」, 跟合思官方弹窗一致。

**合思机器人 · 效率工具**: 列表加 AI 建议筛选 + 多选批量审批(一次最多整页, 机器人按限流分批办理); 「刷新」升级为现场拉合思实时待办(新到的单/被别处审掉的单都即时对齐); 列表加发起人列且可搜索; 审批后樊雪娇等被代审人会收到钉钉知会。

**合思 · 单据详情更全**: 详情弹窗显示「关联申请单」(出差/招待/固定资产申请单, 含申请额度, 点单号可直接打开逐级返回) + 每张申请单下的「行程消费信息」(车次/坐席/区间/出行人/金额/支付方式/报销状态, 企业支付标红提示勿重复报销)。

**AI 审批建议新规则**: 健康证及体检类 — 金额 ≤100 元且发票开票 6 个月内通过, 否则建议驳回。

---

## v1.76.6 (2026-06-11) — YS批量出库工具大升级: 两步向导 + 防重复提交 + 秒级拆单

**两步向导 + 实时进度**: 出库流程拆成「①执行转换 → 点生成/刷新计划复查 → ②执行出库」两步, 中间人工复查目标批次是否都到货、都合格, 复查不过②不解锁(前后端双闸)。执行时居中进度弹窗实时显示「正在执行 X/Y」, 不再盲等。切标签页录入和计划自动暂存, 切回来不丢。

**不合格/废品自动转合格**: 出库一律出合格品——目标批次不够时, 自动按「合格直出 → 合格别批次转换 → 不合格/废品转合格」三档优先级凑数; 批次和状态都要变的, 按用友规则自动拆成两张转换单依次提交。

**整单「全有或全无」**: 一张吉客云单号下任一行缺货, 整张单都不出(不出半张单), 转换也跳过不白改库存; 生成计划后顶部直接汇总「哪几张单缺货、各缺多少」, 不用一行行翻。

**防重复提交 + 强制重发开关**: 同样内容 10 分钟内重复点执行, 系统自动跳过不重复建单(治「超时误报失败→重发→用友重复单据」); 真要原样再建一遍的, 勾选「强制重发」并经红字确认后放行, 用完自动关闭。

**生成计划分钟级→秒级**: 改为按公司批量拉库存清单后本地拆单(实测接口不截断、结果与逐个查询完全一致), 几十个编码的批次从等 3-4 分钟变成几秒出结果; 查询失败自动重试一次, 超大批量也不会再「转圈到死最后报网络错误」。
