...
## v1.76.5 (2026-06-10) — 客服评论数据新模块 + YS批次转换工具增强

**新增「客服部门 → 评论数据」页**: 接入 RPA 抓取的各平台店铺后台评价(天猫/拼多多/抖音/小红书…), 平台一级 Tab + 店铺二级 Tab(含"全部") + 日期筛选, 平铺看评价明细(店铺/时间/订单号/商品名/评价内容/评分), 商品名和评价内容超长悬浮看全。支持导出 Excel(商品名出「原始名」+「客服改后名」两列)。数据每天自动导入(2025-12-29 起约 3880 条)。

**评价数据清洗**: 各平台格式不一, 统一成可用——时间一律转年月日(抖音中文「2026年06月07日」/ 拼多多美式「6/7/26」/ 天猫带「初次评价:」前缀都识别), 订单号剥掉「订单编号：」前缀并按文本存(防 19 位长号精度丢失), 评分按平台适配(天猫文字「负面/中性评价」/ 拼多多抖音数字 / 小红书暂不展示)。

**商品名编辑 + 删除**(客服可做): 商品名太长可逐条改简称、评价可逐条删除。**改名/删除全程不动 RPA 原始数据**(存独立对照表), 每天重导也不会冲掉客服的修改。

**恢复 / 回收站**(仅管理员): 改过的名字能一键「恢复原始」, 删掉的评价能在「显示已删除」开关里找回——撤销权限只在管理员手里, 客服能改不能撤。

**YS 工具「批次/状态转换」增强**: 货品/仓库框改成可搜下拉(本地库取数, 打编码或名字都能搜, 不占用友接口), 仓库支持多选一次查多个仓, 顺手修了字体不统一; 新增「YS工具操作员」角色, 可把工具箱授权给指定的非超管用户。

---

## v1.76.4 (2026-06-05) — 合思单据发票原件一键预览 + 审批规则放宽 + 原料行情准实时

**合思单据详情·发票原件一键预览**: 打开费控单据详情即可直接看发票原件(图片缩略图 / PDF 内嵌全屏), 不用再下载。发票列表加「所属费用」列 + 按费用筛选(一张单几十张票, 一眼看清哪些挂在过路费、哪些挂在委外费下), 发票号码可点看大图, 火车票带出行信息(车次/座位等级自动判定), 发票类型代码补全中文(数电、通行费、机票、区块链等 12 种)。配套一批预览体验优化: 弹窗放大、PDF 整页显示不用拖、阅读器工具条/缩略图栏隐藏只剩干净发票、列宽对齐不再串列。

**合思审批规则放宽**(樊雪娇日常报销 AI 建议): 分公司签的合同可用主公司主体报销; 消费事由 50 字以内自动通过(去掉 10-50 字人工核档这档); 火车票座位等级自动判定。

**员工职级数据升级**: 改用钉钉人员导出表(覆盖 453 人), 职级映射更准(9 档职级归并 5 档), 审批建议判级更靠谱。

**原料行情总览·盘中准实时**: 交易时段每 5 分钟拉新浪最新价, 涨跌按现价 - 昨结实时算(原来只有收盘日线一天一更); 顺修 K 线图日期轴与缩放滑块重叠。

**RPA 数据监控·历史封存**: 老数据平台已经拿不到、但早已全量入库的历史时间段, 标灰封存不再误报「缺数据异常」, 监控页只盯真正该补的近期缺口。

**多标签页带部门前缀**: 跨部门同名页(店铺看板 / 货品看板等)标签自动加部门前缀, 不再撞名分不清是哪个部门的页。

---

## v1.76.3 (2026-06-04) — 特殊渠道价格表自助维护 + 质检预警新模块(来料检验)

**特殊渠道价格表自助维护**: 电商/即时零售「特殊渠道调拨对账」页右上角加「价格表」按钮, 弹窗按渠道(京东/猫超/朴朴)分 Tab 填价/改价(保存二次确认), 缺价商品标红在前。保存后端一条龙: 存价 → 重算该商品已发调拨单金额 → 清看板缓存 → 记操作日志。同一货品跨渠道可填不同价(价格表 UK=渠道+商品, 保存/重算按渠道隔离)。新增 `ecommerce.special_channel_allot:edit` 权限(默认仅超管, 角色权限页可放权)。import-channel-price 去掉整渠道 DELETE 改纯 upsert, Excel 批量导与页面手填两边并存不冲突。(背景: 京东缺价商品销售额按 0 算, 6/4 补回松茸鲜素蚝油 360g 一例)

**质检预警新模块**(供应链管理): 对接用友 YonBIP 来料检验单(`/yonbip/QMS_QIT/inspectorder/list`, billnum=来料检验; 每天 09:40 自动同步 BI-SyncYSInspection, 回灌 2025 至今 4189 张到 `ys_inspection`, inspect_result=2=不合格, 来料不屏蔽香松)。页面=**到货单视角**: KPI(不合格单/已检/不合格率/问题到货批次) + 问题供应商排行 + 不合格率月度趋势(近12月) + 到货质检清单(按到货单分组, 有问题排前, 受控分页一页20, 「查看详情」弹窗看这批货下每张检验单合格/不合格+处理方式)。来源分采购到货/委外到货。新增 `supply_chain.qc_alert:view` 权限(默认供应链/管理层/超管)。注: YS 用单一"统一检验项目"+只记判定/处理, 无文字不合格原因。

---

## v1.76.2 (2026-06-04) — 计划看板整页"调拨当销售"口径统一 (GMV/周转/采购建议, 经二审)

**根因**: 京东(清心湖自营)/猫超(寄售)/朴朴这 3 个特殊渠道靠调拨单发货当销售, 但吉客云月销量与销售汇总账都不含调拨, 它们的销售挂自己平台仓不在计划 8 仓 → 计划看板整页(GMV/周转/缺货/高库存/采购建议/库存监控)系统性漏算这部分, 导致靠调拨走量的畅销货被误判滞销/高库存、采购建议偏保守。跑哥拍板整页口径统一。

**钱侧**(并入调拨金额, 纯加法): GMV KPI +约 406 万/月(5 月实测 3948→4354 万); 渠道环比 split(电商+京东猫超 / 即时零售+朴朴, 上月去年同窗口同并保证 MoM/YoY 公平); 品类销售饼; 月度趋势 — 全部并入保证同页合计一致。新建 loadPlanAllot helper(按 10 核心品类 + 3 渠道聚合)。

**件数侧**(并入调拨件数到周转分母): 高库存判定(畅销调拨货移出, 素蚝油周转 4624→49 天)、周转天数 KPI、品类周转、采购建议日均(修"系统劝你别补热卖货", 且放宽 WHERE 让纯调拨货进清单+补回被漏计的调拨仓库存)、库存监控页(滞销误标修复)。决策: 缺货/单仓缺货不并(货已调拨发出=到位, 调拨无仓维度)。

**经二审(40 agent)修复**: 品类周转售罄货 NULL 污染(日销成本被清空致周转虚高)、库存监控滞销卡片vs下钻一致、渠道/品类合计兜底、前端滞销标签识别调拨在售。落实 6/3 记录的计划看板 GMV 漏算待修(project_plan_dashboard_allot_gap)。

---

## v1.76.1 (2026-06-04) — 线下销量预测算法优化 (春节按去年同期, 经 2 轮二审)

**线下销量预测改混合版**: 春节月(1/2月)预测改按「去年同月 × 大区增长」推算 — 线下经销商节前集中囤货是稳定规律, 比按近期销量更准 (避免近3月均含1月备货高峰把2月带高); 平淡月(3-12)按「近3月均 + 同比 + 环比」加权 × 节假日 × 大区趋势。出数 GetOfflineSalesForecast 与回测 computeBacktestForecast 共用 weightedForecast, 出数/回测/页面说明三层口径强制一致。回测大区合计平均误差 14.1%→8.3%, SKU级春节 50.5%→37.5% (真实数据抽验)。经 2 轮 /code-review 二审。

**配套修复**: 「线下总计」偏离标记基准春节月改用去年同月 (否则误报红↓); 格子 tooltip 去掉对不上的名义公式改摆"近3月均/去年同月"依据 + 加回节假日因子行 + 春节把去年同月提首行; 删误导的季节系数列 (混合版不乘季节系数) 及 Excel 同名列; 出数查询窗口 12月→4月 (1205ms→461ms); 缺项权重按非零项归一化 (不浪费在0值项致偏低); 删死代码 computeSmartForecast/regionMoM/三个 seasonal 输出字段。

---

## v1.76.0 (2026-06-03) — 操作活动面板 + 计划/快递看板加南京自营仓 + 月度趋势全历史 + 调拨销售改按审核日

**操作活动面板**: 个人中心新增「我的活动」(操作统计 今日/近7/近30/总/活跃天 + 近半年热力图 + 最近15条记录); 系统设置→在线用户新增「全员活动」页签 (全员操作总表 + 点行下钻看个人活动 Drawer)。数据来自操作审计日志纯只读聚合, 用户ID只从登录态取防越权; 接口返回错误信封时面板显示0/空不白屏。(UserActivityPanel.tsx / user_activity.go)

**计划/采购/库存/快递看板加南京自营仓 (7→8仓)**: 退役的「南京仓库成品-公司仓-自营」(2025上半年月发20多万单的大仓, 2025-08退役) 加回供应链白名单 planWarehouses, 让南京销售/发货历史趋势连续。物化预聚合表 warehouse_flow_summary 全量重建 (18月, 实时 vs 物化 2025-03 订单数 688184=688184 验证一致), 14处计划看板 tooltip + 采购 + 快递文案 + 地图坐标同步 7→8。buildPlanWarehouseFilter 动态占位符 (原写死7个问号)。

**计划看板月度趋势恢复全历史**: 月度销售趋势从"近13个月"恢复显示全部历史月 — 改读月度物化表 sales_goods_summary_monthly (口径实测=日表同值), 品类 JOIN 派生表替代慢子查询 (9秒→1.3秒)。救火期为防原日表全表扫173秒超时临时砍成13个月, 现根治。

**特殊渠道调拨对账加审核时间列**: 京东/猫超/朴朴调拨对账页新增「审核时间」列 (创建→审核→入库 时间线)。

**特殊渠道调拨销售改按审核日确认 (口径变更, 走 /code-review 二审)**: 调拨销售统计日期从"入库完成日"改按"审核通过日" (平均提前18.6天/71%单跨月, 更及时); status审核通过(已审/已关闭/已完成)即确认, 含已审在途单 (+9单/+115万)。历史2165单已重算, 综合看板/部门页/对账页三处口径统一 (对账页原按入库日过滤会差账, 一并改)。sync 后自动清缓存。受此影响这几个渠道历史月度金额有相应调整 (口径优化非数据错误)。

---

## v1.75.21 (2026-06-02) — 工作台多页签 + 用友工具箱 (批量出库 / 批次转换 / 状态转换)

**多页签**: 工作台顶部加了浏览器式标签栏——打开过的页面排成一排标签, 点标签即切换 (SPA 不重载)、可关闭 / 关其他 / 关全部, sessionStorage 持久化刷新不丢; 选中标签主题蓝高亮 (浅蓝底 + 顶部蓝条 + 蓝字加粗)。不做 keep-alive (看板满屏 ECharts, display:none 缓存会致图表尺寸错乱), 切换走正常路由重渲染。(TabsNav.tsx)

**用友工具箱**: 右上角新增「工具箱」入口 (ToolboxMenu), 「YS工具」二级子菜单聚合用友小工具, 权限 system.yonbip:use (超管单人用); 系统设置菜单移除原「用友批量出库」入口, 改由工具箱进。

**批次 / 状态转换工具**: 新页面 /system/batch-convert, 一比一移植桌面 yonbip_api——查现货 (YS 现存量) → 选行 → 批次转换 (A批次→B批次) / 状态转换 (合格↔不合格) → 清单 → 二次确认执行 (建形态转换单 morphologyconversion/save + 自动审核, 写用友不可逆)。后端 yonbip_convert.go 复用 outbound.go 的 MorphologyConversionSave / Audit / QueryStockByCondition。

**验证**: 后端 go build + 9 单测全过; 蓝军审 yonbip_convert.go 红线逻辑, 修 P0 (执行后清空清单防重复建单, 转换单无幂等键) + P2 (qty 取整对齐 Python / 状态 doc 无条件带); playwright 实测多页签 (打开/切换/关闭/关当前跳相邻/刷新保持) + 批次转换 (查现货联通/弹窗/加清单) 全通过, 真执行留人工验。

---

## v1.75.20 (2026-06-01) — 综合看板默认视图秒开 (缓存按权限共享 + 后台预热)

**问题**: 综合看板冷查要在 177 万行销售汇总上跑十几条聚合, 打开要等 2~3 秒; 缓存又是按"每个人 + 每组参数"分的, 命中率低, 同步一刷就被清, 大家常撞冷查。

**修复**: 两招。① 缓存改成按"数据权限范围"共享 (scopeCacheSig = 超管标志 + 部门/平台/店铺/仓库/域 5 维)——同权限的人看到的数据本就一样, 共用一份缓存, 一个人焐热全组秒开 (权限校验在缓存之前, 不会串数据)。② 后端 ticker 每 90 秒把各去重权限范围的默认视图预先算好填缓存 (overview_prewarm.go), 用户打开即热。实测默认视图打开从 ~6 秒降到 ~9 毫秒。切到不常看的历史月份首次仍需冷查 (数据量大), 但同范围再查也走缓存变快。

**验证**: 蓝军 agent + /code-review 双审 (0 P1, 修 1 处内层缓存 staleness); 5 条回归测试证明同 scope 共享 / 异 scope 隔离不串数据; playwright 实测 9ms 热 / 6274ms 冷 / 6ms 再查。

---

## v1.75.19 (2026-05-29) — 客服满意率改"整体口径"(修评价量小的店满意率虚低)

**问题**: 部分店铺(评价量小的, 如抖音"松鲜鲜调味品旗舰店")在客服总览看到的满意率明显偏低(显示约 80%), 跟平台后台的店铺整体满意率(约 99%)对不上。

**原因**: 看一段时间的满意率时, 系统把"每天的满意率"等权简单平均。评价量小的店, 某天只有 1 条评价、碰巧是差评就算当天 0 分, 却跟有几十条评价的天权重一样, 把整段平均值严重拉低。

**修复**: 满意率改成按"评价量"加权 = 总好评 ÷ 总评价, 跟各平台后台店铺整体口径一致。评价量大的店几乎不变(抖音官方店 98.2%→98.4%), 评价量小的店回归真实(抖音调味品 80.4%→99.3%)。客服总览/趋势/平台/店铺各视图同时生效。本次只调满意率, 其它指标口径不变。

---

## v1.75.18 (2026-05-29) — 心智渗透看板骨架(预览版) + RPA 监控页秒开

收尾 5/28-5/29 两笔散活, 合并发版。

**品牌中心新增"心智渗透"看板 (骨架预览版)**
新增一级模块, 含 4 个页面 (总览 / 知名度 / 好感度 / 心智绑定), 围绕"减钠"品牌心智的三层健康度 (知名度 / 好感度 / 心智绑定) 搭出框架。
当前为**骨架预览版, 显示示意数据** (页面顶部已标注提示), 真实数据待 RPA 团队补加 5 个数据文件 + 业务录入减钠 SKU 清单等到位后回填。
43 个指标已按"IT 能不能拿到数据"分级标注 (11 可立即接 / 5 缺业务输入 / 4 等上游 / 23 暂不可)。
**暂不发全员通知**, 先供品牌中心预览布局、提反馈。

**RPA 数据采集监控页 — 打开秒开**
之前: 监控页偶尔打开要卡约 11 秒 (缓存过期时会触发一次全盘重新扫描)。
现在: 后端每 5 分钟自动在后台刷新一次扫描缓存, 缓存常新, 打开就是秒开。前端逻辑不变。

---

## v1.75.17 (2026-05-27) — AI 建议口径优化: 备用金冲抵 + SSC 未匹配不冤枉

樊雪娇日常报销单 AI 建议两处算法收紧, 避免业务合理场景被误判驳回:

**报销金额 vs 支付金额不一致 (规则 13-⑤)**
之前: 报销金额 ≠ 支付金额 → 直接给"驳回"建议
现在: 报销金额 > 支付金额 (备用金/借款冲抵的真实业务场景) → 改"转人工"提示, 让审批人核实是否是冲抵
真正异常 (支付金额 > 报销金额) 还是驳回

**SSC 表未收录员工的出差补贴/住宿标准 (规则 11 / 规则 7-3)**
之前: 花名册没匹配上 → 按"主管和其他"档算, 超额 = 驳回
现在: 花名册没匹配上 → 还是按"主管和其他"档算上限, 但**超额给"转人工"** + 提示"SSC 未匹配, 请人工核职级"
原因: 公司还有约 400 名员工未在 SSC 表 (审批职位表), 里面如果有未维护的总监/经理, 报合理金额会被冤枉

**dry-run 验证**: 樊雪娇 47 单, 通过率从 0%→2.1%, 转人工从 21%→30%, 驳回从 70%→68%. 之前误判驳回的"4-5月差旅补贴车票报销"等单已正确改为转人工。

---

## v1.75.16 (2026-05-27) — 樊雪娇 AI 建议加规则 8/10/11/12/13 + 7-3 fallback + 字段精度修

樊雪娇日常报销单 AI 建议规则一波打通到 13:

规则 8 (拦截): 发票审核 4 项
- 8-1 抬头: 发票购买方名称必须 = 所属公司 (raw_json.法人实体 反查 hesi_legal_entity_invoice_info)
- 8-2 税号: 购买方税号必须 = 所属公司税号 (开票资料 PDF 字典 44 个公司+税号)
- 8-3 金额: 每个明细的发票合计金额 ≥ 明细报销金额 (加 0.01 容差防浮点精度误判)
- 8-4 开票时间: 距单据提交 ≤1月通过 / 1-3月转人工 / >3月驳回

规则 10 (拦截): 每个明细必须有发票, 除非属于豁免:
- 豁免 A: 研发部门 + 样品 fee_type
- 豁免 B: 出差补贴/市内补贴/餐费补贴
- 豁免 C: 特殊无票截图说明 (合思后台未存字段, 待后期接入)

规则 11 (拦截): 出差补贴单笔 ≤ 出差天数 × 岗位职级标准 (¥/天)
- 总裁 200 / 副总裁 150 / 集团总监 100 / 集团经理 80 / 主管和其他 60
- 半天按一天计算 (ceil)
- SSC 表只覆盖审批职级 240 人, 普通员工 fallback "主管和其他" 60/天

规则 12-1 (转人工): 自驾报销 fee_type 转人工核 KM × 车型 (油 ¥0.7/KM, 电 ¥0.6/KM)
- 合思 raw_json 无车型字段, AI 无法自动判油/电, 给审批人参考

规则 12-2: 消费事由长度 ≤10 字通过 / 10-50 字转人工 / >50 字驳回

规则 13 (拦截): 单据字段必填校验
- 13-② 部门为品牌中心 → u_品牌中心必选 不能空
- 13-③ 部门为研发中心 → u_研发中心必选 不能空 (递归判 parent_id 含"研发")
- 13-⑤ 报销金额 = 支付金额 (0.01 容差)
- (备注 / 附件 跑哥拍板"只是字段说明", 不强制必填)

数据基建新增:
- hesi_legal_entity_invoice_info 表 (44 法人实体抬头+税号, 从开票资料 PDF 自动同步, 异体字 normalize)
- hesi_employee_contract_company.position 字段已扩展 fallback 逻辑 (规则 7-3 + 11 共用)

修复 + 优化:
- 规则 7-3 (住宿) + 规则 11 (补贴): SSC 表 241 人外的提交人, fallback "主管和其他" 档而非转人工 (跑哥拍板, 提示带"花名册未匹配, fallback")
- 规则 5 字典错: ID01MBFnQppBWn 实际是"业务宣传费"而非招待子类, v1.75.15 已修
- 规则 8-3 浮点精度: ¥146.74 < ¥146.74 误判 → 加 0.01 容差
- ruleInvoiceChecks 行号显示: 之前显示 [0] (raw_json.detailId 字段路径错), 改 LEFT JOIN hesi_flow_detail 拿真实 detail_no
- 规则 8-3 触发条件: 改成 raw_json.details[].feeTypeForm.detailId 而不是顶层 detailId

樊雪娇 43 单实测分布: 0 同意 / 33 驳回 / 10 转人工 (规则覆盖率高, AI 建议触发面广)

延期项 (留下次):
- 规则 9 (付款截图金额): 合思 raw_json 无截图金额字段, 等接 OCR 后做
- 规则 12-1 自驾 KM 自动算: 等接合思商旅 API 拿行车记录数据后做

---

## v1.75.15 (2026-05-27) — 樊雪娇 AI 建议加规则 7 (出差差旅) + 修规则 5 字典错

樊雪娇日常报销单 AI 建议规则 7 全套上线 (3 层判定):

规则 7-1 (拦截): 费用明细含交通及差旅费 (飞机/火车/汽车/住宿费/差补/过路费等 15 类) 必须关联出差申请单, 报销金额 ≤ 申请单金额合计.

规则 7-2 (转人工): 飞机/火车/汽车/客车/其他交通明细 → AI 建议自动转人工, 提示审批人核座位等级 (跑哥规则: 二等座及以下, 飞机经济舱). 因合思后台未存舱位字段, 凭票报销时 AI 无法自动判定, 给人工参考.

规则 7-3 (拦截): 住宿费单晚价 ≤ 城市分级 × 岗位职级标准 (5 档 × 4 tier 矩阵, 来源集团费用报销管理制度 V7.0):
- 一线 4 / 新一线 33 / 二线 31 城市 + 国内其他兜底
- 总裁/副总裁/集团总监/集团经理/主管和其他 5 档 (集团 SSC 职位表 240 人覆盖)
- 同住上浮 20% (按职位高者标准)
- 提交人花名册无职级数据时, 转人工提示无法自动判定

修规则 5 字典错: 字典里把"业务宣传费"误标"业务招待子类", 现已剔除.

新增数据基建:
- 住宿城市分级表 (一线/新一线/二线/其他, 68 城市)
- 员工岗位职级字段 (从 SSC 职位表同步 240 单匹配 + 1 重名)

樊雪娇 43 单实测分布: 8 同意 / 25 驳回 / 10 转人工.

---

## v1.75.14 (2026-05-27) — 樊雪娇日常报销单 AI 建议 + 详情体验大升级 + 全库发票号补全

合思机器人 (个人中心) 和 费控管理 联动上线大批改进:

樊雪娇日常报销单 AI 审批建议 (6 条规则, dry-run 模式):
- 发起人部门必须末级 / 报销借款部门必须末级
- 收款信息必须是银行账户 (微信/支付宝/钉钉钱包驳回)
- 所属公司 = 钉钉花名册合同公司
- 业务招待费必须关联招待费用申请单 + 金额不超
- 固定资产必须关联固定资产申请单 + 金额不超
- 仅樊雪娇审 + 日常报销单模板触发, 列表 "AI 建议" 列 + 详情 Modal 部门 inline 红 Tag

单据详情体验:
- 加 "收款信息" 块 (户名/方式 Tag/开户行/账号)
- 费用明细加 "费用类型" 列 (合思字典反查名字, 不再显 ID)
- 行号改连续 1-N (合思 detailNo 重复跳号不再误导)
- 发票列改 "N 张" 显示张数
- 长明细表内部滚动 + 表头固定 (36+ 行不撑出 Modal)
- 发票 Tab 未识别发票号显 "未识别" 标记 + 用明细金额/原因兜底

全库发票号补全 (sync-hesi 旧 bug 修):
- 修前: 只识别 "发票主体/非税收入类票据" 2 种类型字段
- 修后: 通用匹配所有 13 种 (机打/出租车/火车/机票/通行费/医疗/...)
- 一次同步补全 3555 条 invoice 数据, 所有报销单详情发票号显示正常

新建数据基建:
- 合思部门树 hesi_department (511 部门) + 每天 04:30 同步
- 合思收款方 hesi_payee_info (2120 收款人, BANK/ALIPAY/...) + 每天 04:40 同步
...
变更:
- 详情弹窗顶部去掉「凭证明细」Tab (4 个 Tab 简化)
- 凭证状态行: "已生成" + 「查看凭证」link 按钮 (仅 voucherDetail 非空时显)
- 子 Modal 复用原 Tab 渲染逻辑: 凭证表头 (Descriptions) + 分录表 (Table)
- 同步改 HesiBot.tsx

UI 更清晰: 看凭证 = 显式动作, 不占顶部 Tab 位.
...
加 Tooltip 文案: "财务做账状态. 已生成 = 合思生成会计凭证后自动同步到用友, 财务做完账; 未生成 = 还没做账 (单据审批完了但财务那边还没记到账本)."

同步改 HesiBot.tsx, 两个入口一致.

---

## v1.75.5 (2026-05-26) — 单据详情字段自适应隐藏 (支付金额/报销金额/凭证状态/支付时间)

**业务背景**: 跑哥反馈出差申请单弹窗里显示 "支付金额 -" "报销金额 -" "凭证状态 -" "支付时间 -" 全是空, 信息密度差.

**数据驱动分析** (按 form_type 字段填充率):
- expense (报销 10756 单): 91% / 99.9% / 100% / 59% — 都该显
- requisition (申请 11109 单): **0.05% / 0% / 25% / 0.02%** — 该藏
- loan (借款 1348 单): 100% / 0% / 96% / 96% — 看具体字段
- custom (商城 315 单): 0% / 0% / 2% / 0% — 该藏

**规则** (跟 v1.75.2 Tab 自适应思路一致):
- expense 类: 4 字段始终显 (即使空, 作为业务异常警告 — 报销单没凭证状态=数据缺失)
- 其他类: 仅 value 非空才显 (避开 99% 都是 "-" 的噪音)
...
**实测**:
- 出差申请单 S26002768: 10 行 → **8 行** (4 个空字段全藏), 信息密度提升 20%
- 报销单 B26003044: 仍显示 4 字段 (业务警告)

同步改 HesiBot.tsx 审批界面.

---

## v1.75.4 (2026-05-26) — 付款单排除主体校验 (主体语义不同)

**业务背景**: 跑哥反馈付款单（票到付款/票到核销）不需要校验主体. 因为付款单的"法人实体"=被付款方/费用归属公司, 不是申请人本人的合同公司, 字面对比无意义.

**变更**: hesi.go 加 skip list `[ID01KgaO6dcZtR (付款单)]`, 满足 form_type=expense 但匹配 skip list 的跳过校验.
...
实测:
- 付款单 B26003170 entityCheck 空 (未校验, 符合预期)
- 日常报销单 B26003044 entityCheck=mismatch (仍校验, 应"南京分公司")

未来如果还有别的模板要跳过, 在 `skipEntityCheckSpecPrefixes` 数组里加一行 specPrefix 即可.
