...
// MyFeedback 用户查看自己的反馈
// GET /api/feedback/my
func (h *DashboardHandler) MyFeedback(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
		writeError(w, 405, "method not allowed")
		return
	}
	...
	rows, err := h.DB.Query(
		`SELECT id, title, status, reply, replied_by, created_at FROM feedback
	...
	for rows.Next() {
		...
		rows.Scan(&item.ID, &item.Title, &item.Status, &item.Reply, &item.RepliedBy, &createdAt)
		item.CreatedAt = createdAt.Format("2006-01-02 15:04:05")
		list = append(list, item)
	}
	if list == nil {
