...
// GetWarehouseFlowSKU 单 SKU 下钻: 看它的省+仓分布
//   GET /api/warehouse-flow/sku?ym=YYYY-MM&goods_no=XXX&metric=...
func (h *DashboardHandler) GetWarehouseFlowSKU(w http.ResponseWriter, r *http.Request) {
	ym, err := resolveYM(h.DB, r.URL.Query().Get("ym"))
	if err != nil {
		writeError(w, 400, err.Error())
		return
	}
	goodsNo := strings.TrimSpace(r.URL.Query().Get("goods_no"))
	if goodsNo == "" {
		writeError(w, 400, "goods_no required")
		return
	}
	flt := parseFlowFilter(r, ym)
	flt.skuNo = goodsNo // 强制走精确 sku 过滤
	where, args := flt.buildWhere()
	...
	// SKU 名
	var skuName string
	h.DB.QueryRow(fmt.Sprintf(`SELECT goods_name FROM %s WHERE goods_no = ? LIMIT 1`, goodsT), goodsNo).Scan(&skuName)
	...
	provSQL := fmt.Sprintf(`
		...
		%s
		WHERE 1=1 %s
		GROUP BY prov ORDER BY orders DESC`, provinceNormSQL, allMetricsSelect, joins, where)
	byProv := []rowMM{}
	if rs, err := h.DB.Query(provSQL, args...); err == nil {
		defer rs.Close()
		for rs.Next() {
			var p rowMM
			if rs.Scan(&p.Name, &p.Orders, &p.Packages) == nil {
				byProv = append(byProv, p)
			}
		}
	}
	...
	whSQL := fmt.Sprintf(`
		...
		%s
		WHERE 1=1 %s
		GROUP BY t.warehouse_name ORDER BY orders DESC`, allMetricsSelect, joins, where)
	byWH := []rowMM{}
	if rs, err := h.DB.Query(whSQL, args...); err == nil {
		defer rs.Close()
		for rs.Next() {
			var p rowMM
			if rs.Scan(&p.Name, &p.Orders, &p.Packages) == nil {
...
// GetWarehouseFlowSKUList SKU 候选, 用于下拉/搜索
//   GET /api/warehouse-flow/sku-list?ym=YYYY-MM&keyword=...&limit=50
func (h *DashboardHandler) GetWarehouseFlowSKUList(w http.ResponseWriter, r *http.Request) {
	ym, err := resolveYM(h.DB, r.URL.Query().Get("ym"))
	if err != nil {
		writeError(w, 400, err.Error())
		return
	...
	whereKw := ""
	if kw != "" {
		whereKw = " AND (goods_name LIKE ? OR goods_no LIKE ?)"
		args = append(args, "%"+kw+"%", "%"+kw+"%")
	}

	sqlStr := fmt.Sprintf(`
		SELECT goods_no, MAX(goods_name) AS goods_name,
			COALESCE(SUM(sell_count), 0) AS qty,
			COALESCE(SUM(divide_sell_total), 0) AS amt
		FROM %s
		WHERE goods_no IS NOT NULL AND goods_no != '' %s
		GROUP BY goods_no
		ORDER BY qty DESC
		LIMIT %d`, goodsT, whereKw, limit)
	...
	out := []item{}
	if rs, err := h.DB.Query(sqlStr, args...); err == nil {
		defer rs.Close()
		for rs.Next() {
			var it item
			if rs.Scan(&it.GoodsNo, &it.GoodsName, &it.Qty, &it.Amt) == nil {
				out = append(out, it)
			}
		}
	}
	writeJSON(w, map[string]interface{}{"items": out, "ym": ym[:4] + "-" + ym[4:]})
}
