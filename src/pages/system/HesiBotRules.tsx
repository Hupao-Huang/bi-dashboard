...
const FXJ_AUDIT_RULES: { cat: string; rule: string }[] = [
  ...
  { cat: '先申请后报销', rule: '交通及差旅费要关联「出差申请单」，且金额不超过申请额度（私车公用、过路费除外，无需关联）' },
  ...
  { cat: '发票', rule: '发票要齐全，且抬头、税号、金额、开票时间都对得上（研发样品、出差补贴、私车公用、特殊截图说明 这几类可免发票）' },
  ...
  { cat: '发票', rule: '广告费与"广告/推广"发票双向对应：① 费用类型为广告费时，发票项目名称必须含"广告"或"推广"（开成"印刷品"等会建议驳回）；② 反过来，发票项目名称含"广告"或"推广"但费用类型不是广告费的，也建议驳回' },
];
