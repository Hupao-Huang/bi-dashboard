...
const FXJ_AUDIT_RULES: { ref: string; cat: string; rule: string }[] = [
  ...
  { ref: '7-2', cat: '差旅标准', rule: '座位等级：① 火车票按发票上的坐席自动判——二等座及以下（含硬座、软座、硬卧、二等卧、无座等）通过，一等座、商务座、特等座、一等卧建议驳回，软卧、高级软卧、动卧或坐席识别不出来的转人工核；② 飞机（经济舱）、客车、汽车的座位等级票面上看不到，统一转人工核' },
  ...
  { ref: '10', cat: '发票', rule: '这几类可以免发票：研发样品、出差补贴、私车公用、特殊截图说明、外币（国外）报销（付款截图显示美元/港币/USD 等外币的，视同境外消费、没有中国发票）' },
  ...
  { ref: '19', cat: '付款核对', rule: '付款截图里的实付金额不能比发票合计多出超过 2 元（差几毛手续费、凑整不算）；多出较多会转人工核。付款少于发票（比如差旅有票没截图）正常通过；外币（国外）报销没有可比的中国发票，不做这项核对' },
];
