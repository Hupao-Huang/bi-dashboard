...
const ZJ_AUDIT_RULES: { cat: string; rule: string }[] = [
  { cat: '所属公司', rule: '所属公司（法人实体）必须填写' },
  { cat: '所属公司', rule: '付款事由涉及"阳光天际"时，所属公司必须是杭州松鲜鲜悦伍食品科技有限公司' },
  { cat: '部门', rule: '申请部门必须是末级部门（部门底下不能再挂下级）' },
  { cat: '收款', rule: '收款方式必须是银行账户（填"其他"等非银行账户会建议驳回）；新建收款方时银行账户信息要填齐全' },
  { cat: '收款', rule: '收款方不能与付款的所属公司同名（避免自己付给自己）' },
  { cat: '事由', rule: '付款事由 / 费用明细的消费事由必须填写，且不能出现"合计、小计"等笼统字样' },
  { cat: '客户', rule: '客户信息要选择（非线下业务统一选虚拟客户）' },
  { cat: '费用明细', rule: '至少要有一行费用明细（预付款单"先款后票"在发票未到前可以暂无明细）' },
  { cat: '发票', rule: '发票上的购买方必须是本付款单的所属公司（不一致会建议驳回）' },
  { cat: '发票', rule: '收款方名称要与发票上的开票方一致（对不上转人工核）' },
  { cat: '发票', rule: '发票张数要与单据填写的份数对得上（对不上转人工核）' },
  { cat: '金额', rule: '付款金额不能超过发票金额合计（超出转人工核）' },
  { cat: '合同', rule: '对外付款超过 2 万元，建议上传盖章版合同（未上传只提醒，不驳回）' },
  { cat: '防重复', rule: '同一收款方、相同金额若此前已付过，会提示疑似重复付款并附上原单号，请人工核对（付款单、预付款单都查）' },
];
